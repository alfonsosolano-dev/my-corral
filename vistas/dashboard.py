...
# Importamos ambas funciones desde utils
...
def mostrar(df_lotes, df_gastos, df_ventas, df_prod, df_bajas, temp):
    ...
    # 2. CÁLCULOS DE PIENSO DINÁMICO (Lo que baja solo)
    # stock_real es el total comprado menos lo que ya se han comido según su edad
    stock_real, desglose = obtener_estado_pienso(df_lotes, df_bajas, df_gastos)
    
    # Autonomía basada en el stock real actual
    ...
    # Aquí ahora mostramos el Stock REAL (el que descuenta el consumo diario)
    c3.metric("⚖️ Stock Almacén", f"{stock_real:.1f} kg", 
              delta=f"-{round(sum(desglose.values()), 1)} comidos", delta_color="inverse")
    
    c4.metric("⏳ Autonomía", f"{autonomia} días", 
              delta=f"{consumo_dia:.2f} kg/día", delta_color="off")
    ...
    with col_a:
        if not df_prod.empty:
            fig_prod = px.area(df_prod.tail(30), x='fecha', y='huevos', 
                               title="Evolución Puesta (30d)", color_discrete_sequence=['gold'])
            st.plotly_chart(fig_prod, use_container_width=True)
    with col_b:
        if not df_lotes.empty:
            fig_pie = px.pie(df_lotes, values='cantidad', names='especie', 
                             title="Censo por Especie", hole=0.4)
            st.plotly_chart(fig_pie, use_container_width=True)

    st.divider()

    # 5. DESGLOSE POR ESPECIE (Histórico acumulado)
    st.subheader("📉 Consumo Histórico Acumulado por Especie")
    if desglose:
